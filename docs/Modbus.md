...
“MASK 计算”在 Modbus 场景里通常可能指两类事情：

1. RTU 帧的 CRC16 校验计算。
2. `0x16 Mask Write Register` 的位掩码写寄存器计算。

两者不是同一个概念，工程实现时需要分开处理。

#### 2.2.1 RTU CRC16 校验计算

Modbus RTU 使用 CRC16 校验整帧数据，不包含最终的 CRC 两个字节本身。算法参数通常是：
...
low = crc & 0xFF
high = (crc >> 8) & 0xFF
print(hex(crc), hex(low), hex(high))
```
...
#### 2.2.2 Mask Write Register 掩码计算

功能码 `0x16` 用于按位修改单个 Holding Register。它不会直接覆盖整个寄存器，而是通过 AND Mask 和 OR Mask 组合得到新值。

计算公式：

```text
Result = (Current Value AND And_Mask) OR (Or_Mask AND NOT And_Mask)
```

实现时 `NOT And_Mask` 应限定在 16 bit 范围内，例如 `(~And_Mask) & 0xFFFF`。

用途示例：

- 只修改某些 bit，保留其他 bit。
- 控制状态字中的某个标志位。
- 避免读改写过程中覆盖设备内部更新的其他位。

工程注意点：

- `0x16` 并非所有设备都支持。
- 如果设备不支持，通常需要使用 `0x03` 读取当前值，然后在客户端侧完成位运算，再用 `0x06` 或 `0x10` 写回。
- 读改写方案要考虑并发更新风险，尤其是状态寄存器被设备内部逻辑频繁修改时。
...
| 告警码 | `0x04` | `30010` | `uint16` | `1` | - | 按位解析告警 |
